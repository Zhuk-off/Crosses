...
export function UIModal({ width = "md", className, children, isOpen=false, onClose }) {
  ...
  const modal =  (
    <div
      className={clsx(
        "fixed inset-0 bg-slate-900/60 backdrop-blur-sm py-10 overflow-y-auto",
        className
      )}
      onClick={handleClick}
    >
      ...
      data-id='modal'
        className={clsx(
          "relative bg-white rounded-lg min-h-[320px] mx-auto",
          "flex flex-col",
          {
      ...
      >
        <button
          ...
          hover:bg-white/40 transition-colors bg-white/10 
...
UIModal.Footer = function UIModalFooter({ children, className }) {
  return (
    <div className={clsx("p-6 flex mt-auto gap-4 justify-end", className)}>
      {children}
    </div>
  );
};
